...
#include "qogonxcursor.h"
#include "ogon-cursor.h"
#include "../qogonwindowmanager.h"
#include <ogon/backend.h>

QOgonCursor::QOgonCursor(QOgonWindowManager *wm) : mWindowManager(wm), mLastCursor(Qt::ArrowCursor) {
	...
	mCursorTheme = ogon_cursor_theme_load(cursorTheme.constData(), cursorSize);
	initCursorMap();
}

QOgonCursor::~QOgonCursor() {
	if (mCursorTheme)
		ogon_cursor_theme_destroy(mCursorTheme);
}
...
void computeMaskAndData(struct ogon_cursor_image *image, uint32_t *data, char *mask) {
	unsigned int p;
	uint32_t *src;
...
void QOgonCursor::restoreLastCursor() {
	changeCursor(&mLastCursor, NULL);
}

//static int cursorIndex = 0;

void QOgonCursor::changeCursor(QCursor *cursor, QWindow *window) {
	Q_UNUSED(window);
	...
	if (newShape < Qt::BitmapCursor) {
		...
		struct ogon_cursor *ogonCursor = requestCursor((OgonCursor)newShape);

		image = ogonCursor->images[0];
		...
		msg.xPos = image->hotspot_x;
		msg.yPos = image->hotspot_y;
		msg.xorBpp = 32;
		...
		mWindowManager->setCursor(&msg);
	}
}

/* stolen from qtWayland */
void QOgonCursor::initCursorMap()
{
...
struct ogon_cursor *QOgonCursor::requestCursor(OgonCursor shape) {
	struct ogon_cursor *cursor = mCursors.value(shape, 0);
	if (!cursor) {
		QList<QByteArray> cursorNames = mCursorNamesMap.values(shape);
		foreach (QByteArray name, cursorNames) {
			cursor = ogon_cursor_theme_get_cursor(mCursorTheme, name.constData());
			if (cursor) {
